...
- 报告输出：Markdown、JSON 或 JUnit XML，可写入文件并自动创建父目录。
- CI 门禁：JUnit 报告可直接进入 GitHub Actions、GitLab、Jenkins、Buildkite 等测试报告面板。
...
embedding-cache-inspector inspect jsonl examples/sample.jsonl \
  ...
  --required-metadata source,language \
  --format junit \
  --report reports/embedding-cache.junit.xml
```
...
JUnit 报告会把每条 finding 写成一个 testcase：

- `error` 会变成 `<failure>`，适合阻断 CI。
- `warning` 会写入 `<system-out>`，适合在测试面板里展示但不一定阻断。
- 没有 finding 时会生成一个通过的 `cache.clean` testcase。
...
- uses: actions/setup-python@v5
  with:
    python-version: "3.12"
- run: python -m pip install git+https://github.com/yanqr213/embedding-cache-inspector.git
- name: Inspect embedding cache
  run: |
    embedding-cache-inspector inspect jsonl data/embedding-cache.jsonl \
      --expected-dimension 1536 \
      --required-metadata source,language \
      --format junit \
      --report reports/embedding-cache.junit.xml
- uses: actions/upload-artifact@v4
  if: always()
  with:
    name: embedding-cache-report
    path: reports/embedding-cache.junit.xml
```

如果你只想收集报告但暂不阻断流水线，可以加 `--fail-on never`。
...
- JSON, Markdown, and JUnit XML reporters.
- CI-ready output for test report dashboards.
...
Each finding becomes a testcase. Errors become failures; warnings are written to `system-out`; clean caches emit one passing `cache.clean` testcase.
